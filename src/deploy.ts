/**
 * Deploy rentproof-app contract to a Midnight network (undeployed by default; use --network preview|preprod for public networks).
 *
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy rentproof-app to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  const waitForWalletState = () =>
    Rx.firstValueFrom(
      walletCtx.wallet.state().pipe(
        Rx.filter(
          (s) =>
            isProgressStrictlyComplete(s.shielded.state.progress) &&
            isProgressStrictlyComplete(s.unshielded.progress),
        ),
      ),
    );
  ...
  if (network !== 'undeployed' && networkConfig.faucet) {
    ...
    const initialBalance = await waitForWalletState();
    ...
    if (initialTNight === 0n) {
      ...
      while (true) {
        ...
        const s = await waitForWalletState();
  ...
  const dustState = await waitForWalletState();
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (isDustShortage) {
        const currentState = await waitForWalletState();
